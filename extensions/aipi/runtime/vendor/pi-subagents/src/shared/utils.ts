...
export function getAgentDir(): string {
	const aipiScoped = process.env.AIPI_SUBAGENTS_AGENT_DIR;
	if (aipiScoped === "~") return os.homedir();
	if (aipiScoped?.startsWith("~/")) return path.join(os.homedir(), aipiScoped.slice(2));
	if (aipiScoped) return aipiScoped;
...
export function formatResourceLimitExceeded(input: { agent: string; kind: "maxExecutionTimeMs" | "maxTokens" | "maxToolCalls"; limit: number; observed?: number }): string {
	if (input.kind === "maxExecutionTimeMs") {
		return `Resource limit exceeded for ${input.agent}: maxExecutionTimeMs ${input.limit}ms.`;
	}
	if (input.kind === "maxToolCalls") {
		return `Resource limit exceeded for ${input.agent}: maxToolCalls ${input.limit}${input.observed !== undefined ? ` (observed ${input.observed})` : ""}.`;
	}
