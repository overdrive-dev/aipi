...
async function runSingleAttempt(
	...
	const exitCode = await new Promise<number>((resolve) => {
		...
		const triggerResourceLimit = (kind: "maxExecutionTimeMs" | "maxTokens" | "maxToolCalls", limit: number, observed?: number) => {
		...
		const processLine = (line: string) => {
			...
			if (evt.type === "tool_execution_start") {
				...
				progress.toolCount++;
				if (options.maxToolCalls !== undefined && progress.toolCount > options.maxToolCalls) {
					triggerResourceLimit("maxToolCalls", options.maxToolCalls, progress.toolCount);
					return;
				}
