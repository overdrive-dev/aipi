...
export function computeFileHash(text: string): string {
	...
	// AIPI VENDOR PATCH (Bun -> Node): upstream uses `Bun.hash.xxHash32`, a
	// Bun-only global. The tag is a store-internal fingerprint ("not meaningful
	// outside that store"), so only determinism matters, not the exact
	// algorithm. This is a pure-JS 32-bit FNV-1a over UTF-16 code units with an
	// avalanche finalizer, folded to the low 16 bits — same 4-hex tag shape,
	// no runtime dependency, deterministic across Node processes.
