...
/**
 * Disk-backed {@link Filesystem} using Node's `fs/promises` APIs. The default
 * for CLI use. Paths are accepted as-is; callers responsible for any cwd or
 * jail/sandbox resolution should wrap this with their own subclass.
 *
 * AIPI VENDOR PATCH (Bun -> Node): upstream implements this class with Bun's
 * file APIs (`Bun.file` / `Bun.write`). Rewritten onto `node:fs/promises`
 * (already imported above as `fs`) so the vendored default runs under the
 * Node runtime the AIPI host executes on. Behavior is preserved: `readText`
 * decodes UTF-8, missing paths raise {@link NotFoundError}, and the patcher
 * still owns BOM stripping / line-ending normalization.
 */
